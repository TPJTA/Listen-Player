  ...
  <div class="player">
      ...
      <div class="playerData">
        ...
        <div class="playerDetail">
          ...
          <div class="playerProgress" @click="clickProgress">
            <div class="playerBuffered" :style="`width: ${playerBufferedPosition}%`"></div>
            <div class="playerCurrent" :style="`width: ${playingPosition}%`"></div>
            <div class="progressButton" v-if="!isBuffering" :style="`left: ${playingPosition}%`" @mousedown.stop="progressButtonMousedown"></div>
            <div class="progressBufferingButton" v-else :style="`left: ${playingPosition}%`" @mousedown.stop="progressButtonMousedown">
              <span></span>
              <span></span>
              <span></span>
              <span></span>
      ...
      <div class="playerVoice">
        ...
        <div class="voiceProgress" @click="clickProgress">
          ...
          <div class="progressButton" :style="`left: ${voicePosition}%`" @mousedown.stop="progressButtonMousedown"></div>
        </div>
      </div>
      <div class="playerOther">
        <div class="playerListButton iconfont" @click="setPlayerList">&#xe600;</div>
...
export default {
  ...
  computed:{
    playerBufferedPosition: {
      get() {return this.privatePlayerBufferedPosition},
      set(val) {
        if(val >= 0 && val <= 100) {
    ...
    playingPosition: {
      get() {return this.privatePlayingPosition},
      set(val) {
        if(val >= 0 && val <= 100) {
    ...
    voicePosition: {
      get() {return this.privateVoicePosition},
      set(val) {
        if(val >= 0 && val <= 100) {
    ...
    timeData: function() {
      let songTime = secondsToMinutes(this.songData.duration).join(":")
      let playTime = secondsToMinutes(this.playingTime).join(":")
  ...
  methods:{
    ...
    progressButtonMousedown(event1) {
      ...
      var domClass = event1.currentTarget.parentElement.className
      const moveFu = (event2) => {
    ...
    clickProgress(event) {
      if(event.currentTarget.className === "playerProgress") {
        this.playingPosition = (event.clientX - event.currentTarget.offsetLeft) / event.currentTarget.offsetWidth * 100
        this.$refs.music.currentTime = this.playingTime
      }else if (event.currentTarget.className === "voiceProgress") {
        this.$refs.music.volume = (event.clientX - event.currentTarget.offsetLeft) / event.currentTarget.offsetWidth
    ...
    getbuff() {
      ...
      for(let i = 0; i < music.buffered.length; i++) {
        if(this.$refs.music.buffered.end(i) > this.$refs.music.currentTime) {  
          this.playerBufferedPosition = this.$refs.music.buffered.end(i) / this.$refs.music.duration * 100 
          break
        }
      }
    },
...
.hasPlayerList-enter-active, .hasPlayerList-leave-active {
	transition: all .4s;
}
.hasPlayerList-enter, .hasPlayerList-leave-to {
